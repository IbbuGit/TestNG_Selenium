...
public class PAM_NewLoan_PO extends BasePage {
	...
	@FindBy(xpath = "//td[contains(text(),'to perform a draw on their account?')]")
	private WebElement noDrw_Button;

	// Loan Originated Successful
	@FindBy(xpath = "//input[@type='button']")//input[@name='ok']")//--Ok
	...
	public void clickyesButton(String data) {
		if(isDisplayed(noDrw_Button)) {
			click_No_ButtonLOC(data);
			//click_Yes_ButtonLOC(data);
		}else {
		click_Yes_Button(data);
		click_Ok_Button(data);
